    ...
    <main id="main">
      ...
      <section className="section two-column page-width" aria-labelledby="boundaries-title">
        ...
        <div id="school-plan" className="plan-note">
          ...
          <CheckoutButton className="text-action checkout-action" label="Open the $99 monthly Sociobot checkout" />
        </div>
      </section>
    </main>
  );
...
function WorkspacePage({ section, classId }: { section: WorkspaceSection; classId?: string }) {
  ...
  async function release(id: string, booking: BookingSummary) { if (!window.confirm(`Cancel ${booking.guardianName}'s confirmed place?`)) return; setBusy(true); try { const result = await cancelBooking(id, booking.id); setError(result.offerToken ? (result.deliveryStatus === "email_queued" ? "The selected booking was cancelled. The released-seat email is queued." : "The selected booking was cancelled. Copy the one-click offer below and send it through your school's approved channel.") : "The selected booking was cancelled. No guardian is waiting."); await refresh(); await showBookings(id); } catch (cause) { setError((cause as Error).message); } finally { setBusy(false); } }
  ...
  if (isSignedIn === null) return <main id="main" className="page-width app-main"><h1 tabIndex={-1}>{workspaceHeading(section, "loading")}</h1><LoadingState /></main>;
  if (!isSignedIn) return <main id="main" className="page-width app-main"><p className="eyebrow">School workspace</p><h1 tabIndex={-1}>{section === "classes" ? "Sign in to manage class capacity" : workspaceHeading(section, "signedOut")}</h1><p className="lede">Owners, operators, and viewers use Sociobot’s Microsoft sign-in. Parent booking pages stay public.</p><button className="button primary" onClick={() => void signIn()}>Sign in with Sociobot</button><section className="plan-note"><h2>School plan</h2><p><strong>$99 per school each month.</strong> It includes calendar checks and one-click released-seat offers.</p><p>{emailDelivery === "smtp" ? "This deployment can send offer email." : "This deployment creates a copyable offer for staff to send through an approved channel."}</p><CheckoutButton label="Start the $99 monthly plan" /></section></main>;
  if (!workspace) return <main id="main" className="page-width app-main"><p className="eyebrow">School workspace</p><h1 tabIndex={-1}>{section === "classes" ? "Create your school workspace" : workspaceHeading(section, "empty")}</h1><p className="lede">Set a class capacity and cutoff, then publish a parent booking link.</p><form className="booking-form workspace-form" onSubmit={start}><label>School name<input name="schoolName" required minLength={2} maxLength={100} placeholder="Bright Path Languages" /></label>{error && <p className="form-error" role="alert">{error}</p>}<button className="button primary" disabled={busy}>{busy ? "Creating workspace…" : "Create school workspace"}</button><p className="field-help">Your Entra identity owns this workspace. The browser key only selects it.</p></form><button className="button quiet" onClick={() => void signOut()}>Sign out</button></main>;
  ...
  return <main id="main" className="page-width app-main"><p className="eyebrow">{workspace.schoolName}</p><h1 tabIndex={-1}>Manage class capacity</h1><WorkspaceNavigation active="classes" /><p className="lede">Publish only a count you can reconcile. A mismatch is shown as attention, never changed automatically.</p>{emailDelivery === "not_configured" && <p className="state-explanation" role="status">This deployment does not send email. Cancelling creates a one-click offer below. Copy it and send it through your school's approved channel.</p>}{error && <p className="form-error" role="alert">{error}</p>}
...
function WorkspaceSectionPage({ section, classId, workspace, classes, bookings, offerReceipts, copiedReceipt, operationalMetrics, error, busy, emailDelivery, onCalendar, onPublish, onShowBookings, onRelease, onCopy, onBilling, onDownload, onDelete }: {
  ...
  const headings: Record<WorkspaceSection, { heading: string; lede: string }> = {
    ...
    billing: { heading: "Manage school billing", lede: "The school plan costs $99 each month." },
  ...
  const classDetails = detail ? <article className="class-card"><div className="class-card-heading"><div><h2>{detail.name}</h2><p>{formatStart(detail.startsAt, detail.timezone)} · {detail.timezone}</p></div><strong className={`status status-${detail.availability}`}>{availabilityText(detail)}</strong></div><CapacityRail capacity={detail.capacity} confirmed={detail.confirmed} label={`${detail.confirmed} confirmed, ${detail.openSeats} open`} />{detail.published ? <p><strong>Parent link:</strong> <AppLink href={`/book/${detail.publicId}`}>Open booking page</AppLink></p> : <button className="button primary" type="button" disabled={busy} onClick={() => void onPublish(detail.id)}>Publish parent link</button>}<p>{detail.reconciliationStatus === "attention" ? `Attention: calendar says ${detail.calendarConfirmed}, local ledger says ${detail.confirmed}.` : detail.reconciliationStatus === "matched" ? "Calendar check matches the local seat ledger." : "Waiting for the first automatic calendar check."}</p>{detail.published && <><button className="button secondary" type="button" onClick={() => void onShowBookings(detail.id)}>Choose a booking to cancel</button>{bookings[detail.id] && <ul className="booking-list">{bookings[detail.id].length === 0 ? <li>No confirmed bookings remain.</li> : bookings[detail.id].map((booking) => <li key={booking.id}><span><strong>{booking.guardianName}</strong><small>{booking.guardianEmail}</small></span><button className="button quiet" disabled={busy} onClick={() => void onRelease(detail.id, booking)}>Cancel {booking.guardianName} booking</button></li>)}</ul>}</>}</article> : <StatePanel tone="warning" title="This class is not in the current workspace" detail="Return to classes and choose a class that belongs to this school." action={<AppLink className="button secondary" href="/app">Open classes</AppLink>} />;
  return <main id="main" className="page-width app-main"><p className="eyebrow">{workspace.schoolName}</p><h1 tabIndex={-1}>{copy.heading}</h1><WorkspaceNavigation active={section} /><p className="lede">{copy.lede}</p>{emailDelivery === "not_configured" && section === "waitlist" && <p className="state-explanation" role="status">This deployment does not send email. Copy a durable offer and send it through your school's approved channel.</p>}{error && <p className="form-error" role="alert">{error}</p>}
...
function PrivacyPage() {
  return (
    <main id="main" className="page-width legal-page"><p className="eyebrow">Last updated 28 August 2026</p><h1 tabIndex={-1}>Privacy for bookings and the demo</h1><p className="lede">The public demo is temporary. School bookings use encrypted contact fields and staff access controls.</p>
      ...
      <h2>School booking data</h2><p>The school is the data controller. Sociobot processes guardian names and email addresses to manage places and create requested offers.</p><p>Contact fields are encrypted at rest. They are erased 90 days after collection. Audit records may remain without the contact fields.</p>
