    ...
    <main id="main">
      ...
      <section className="section two-column page-width" aria-labelledby="boundaries-title">
        ...
        <div id="school-plan" className="plan-note">
          ...
          <a className="text-action" href="https://api.sociobot.in/api/v1/products/class-capacity-truth/checkout">Open the $99 monthly Sociobot checkout <span className="sr-only">(external checkout)</span></a>
        </div>
      </section>
    </main>
  );
...
function WorkspacePage() {
  ...
  async function release(id: string, booking: BookingSummary) { if (!window.confirm(`Cancel ${booking.guardianName}'s confirmed place?`)) return; setBusy(true); try { const result = await cancelBooking(id, booking.id); setError(result.offerToken ? (emailDelivery === "smtp" ? "The selected booking was cancelled. One expiring email offer was queued for delivery." : "The selected booking was cancelled. The offer was recorded, but this deployment has no email relay and did not send it.") : "The selected booking was cancelled. No guardian is waiting."); await refresh(); await showBookings(id); } catch (cause) { setError((cause as Error).message); } finally { setBusy(false); } }
  ...
  if (!isSignedIn) return <main id="main" className="page-width app-main"><p className="eyebrow">School workspace</p><h1 tabIndex={-1}>Sign in to manage class capacity</h1><p className="lede">Owners, operators, and viewers use Sociobot’s Microsoft sign-in. Parent booking pages stay public.</p><button className="button primary" onClick={() => void signIn()}>Sign in with Sociobot</button><section className="plan-note"><h2>School plan</h2><p><strong>$99 per school each month.</strong> It includes calendar checks and recorded released-seat offers.</p><p>{emailDelivery === "smtp" ? "Email delivery is configured for this deployment." : "Email delivery is not configured on this deployment. Offers are recorded but not sent."}</p><a className="button secondary" href="https://api.sociobot.in/api/v1/products/class-capacity-truth/checkout">Start the $99 monthly plan <span className="sr-only">(external checkout)</span></a></section></main>;
  ...
  return <main id="main" className="page-width app-main"><p className="eyebrow">{workspace.schoolName}</p><h1 tabIndex={-1}>Manage class capacity</h1><p className="lede">Publish only a count you can reconcile. A mismatch is shown as attention, never changed automatically.</p>{emailDelivery === "not_configured" && <p className="state-explanation" role="status">Email delivery is not configured on this deployment. Cancelling a booking records an offer but sends no email.</p>}{error && <p className="form-error" role="alert">{error}</p>}
...
function RealBookingPage({ publicClassId }: { publicClassId: string }) {
  ...
  async function waitlist(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setPending(true); setError(""); const form = new FormData(event.currentTarget); try { await joinWaitlist(publicClassId, String(form.get("guardianName")), String(form.get("guardianEmail")), key.current); setComplete("You are on the waitlist. The school can record a 24-hour offer if a seat is released. Email requires its configured relay."); } catch (cause) { setError((cause as Error).message); } finally { setPending(false); } }
